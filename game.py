...
from menus.game_over import (
    game_over_screen,
)  # <--- IMPORTADO (en vez de la función local)
from shop import shop_screen  # <--- NUEVO: Importar la tienda
from menus.victory import victory_screen
...
def loop_juego(screen, clock):
    ...
    def load_level(level_index):
        ...
        for r in range(max(game_map.tileheight, game_map.tilewidth)):
            for i in range(-r, r + 1):
                ...
                test_points = [
                    (center_tile_x + i, center_tile_y - r),
                    (center_tile_x + i, center_tile_y + r),
                    (center_tile_x - r, center_tile_y + i),
                    (center_tile_x + r, center_tile_y + i),
                ]
    ...
    while True:
        ...
        if enemy_spawn_timer > 0:
            ...
            if enemy_spawn_timer <= 0:
                safe_zone = player.rect.inflate(
                    cfg.TILESIZE * 8, cfg.TILESIZE * 8
                )  # Zona más grande
                ...
                for enemy_type, count in pending_enemies_to_spawn:
                    for _ in range(count):
                        ...
                        while not spawned and attempts < max_attempts:
                            ...
                            if (
                                0 <= tile_y < len(game_map.data)
                                and 0 <= tile_x < len(game_map.data[0])
                                and game_map.data[tile_y][tile_x] != "1"
                            ):
                                ...
                                if not safe_zone.colliderect(enemy_rect) and not any(
                                    wall.rect.colliderect(enemy_rect) for wall in walls
                                ):
                        ...
                        if not spawned and attempts >= max_attempts:
                            print(
                                f"Advertencia: No se pudo spawnear enemigo {enemy_type} después de {max_attempts} intentos"
                            )
        ...
        if (
            not enemies
            and not exit_group
            and not pending_enemies_to_spawn
            and enemy_spawn_timer <= 0
        ):
            if exit_pos:
        ...
        for s in all_sprites:
            if hasattr(s, "draw_health_bar"):
                s.draw_health_bar(game_surface)
            if hasattr(s, "draw_hp"):
                s.draw_hp(game_surface)
